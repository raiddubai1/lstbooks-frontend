      ...
      <div className="w-full md:w-80 bg-white dark:bg-gray-800 border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-700 flex flex-col max-h-48 md:max-h-none overflow-hidden">
        ...
        <div className="p-3 md:p-4 border-b border-gray-200 dark:border-gray-700">
          ...
          <button
            ...
            className="w-full bg-blue-600 text-white px-3 md:px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 text-sm md:text-base"
          >
        ...
        <div className="flex-1 overflow-y-auto">
          {sessions.length === 0 ? (
            <div className="p-3 md:p-4 text-center text-gray-500 dark:text-gray-400">
              <MessageSquare className="w-8 h-8 md:w-12 md:h-12 mx-auto mb-2 opacity-50" />
              <p className="text-xs md:text-sm">No chats yet</p>
              <p className="text-xs mt-1 hidden md:block">Start a new conversation!</p>
            </div>
          ) : (
            ...
            <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-3 md:p-4">
              <form onSubmit={sendMessage} className="flex gap-2 md:gap-3">
                <input
                  ...
                  className="flex-1 px-3 md:px-4 py-2 md:py-3 text-sm md:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                  disabled={sendingMessage}
                />
                <button
                  type="submit"
                  disabled={!message.trim() || sendingMessage}
                  className="bg-blue-600 text-white px-4 md:px-6 py-2 md:py-3 rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base"
                >
...
const MessageBubble = ({ message }) => {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[85%] md:max-w-3xl px-3 md:px-4 py-2 md:py-3 rounded-lg text-sm md:text-base ${
          isUser
      ...
      >
        ...
        <p className={`text-xs mt-1 md:mt-2 ${isUser ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}>
