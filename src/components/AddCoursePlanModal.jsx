...
const AddCoursePlanModal = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post('/course-plans', formData);
      onSuccess();
      onClose();
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to create course plan');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            >
              {loading ? 'Creating...' : 'Create Course Plan'}
            </button>
          </div>
        </form>
      </div>
...
export default AddCoursePlanModal;
